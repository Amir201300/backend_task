...
<script type="text/javascript">
    ...
    function deleteFunction(id,type) {
        if (type == 2 && checkArray.length == 0) {
            ...
            url =  "/Manger/Packages/destroy/" + id;
            deleteProccess(url);
        }else{
            url= "/Manger/Packages/destroy/" + checkArray + '?type=2';
            deleteProccess(url);
            checkArray=[];
        }
    }
...
<script>
    function ChangeStatus(status,id) {
        ...
        $.ajax({
            url : '/Manger/Packages/ChangeStatus/' +id +'?status='+status,
            type : 'get',
            success : function(data){
                $.toast().reset('all');
                table.ajax.reload();
